...
/**
 * AccueilFrontController — Main front-office landing page.
 *
 * Features:
 *  - Product catalogue with filtering (category, price, search, promo, new)
 *  - Cart (panier) badge & navigation
 *  - Order history
 *  - Réclamations (front)
 *  - Notifications badge
 *  - Blog public view (PharmaX articles) ← NEW INTEGRATION
 *  - Back-office access via "Connexion" button
 */
public class AccueilFrontController {
    ...
    public void initialize() {
        ...
        updateNotifBadge();
        Timeline notifTl = new Timeline(new KeyFrame(Duration.seconds(15), e -> updateNotifBadge()));
        notifTl.setCycleCount(Animation.INDEFINITE);
        notifTl.play();

        // Setup Admin Dashboard button visibility
    ...
    public void showPanier() {
        loadSubView("/frontoffice/panier.fxml", ctrl -> {
            if (ctrl instanceof PanierController pc) {
                pc.setOnContinuerAchats(this::retourCatalogue);
            }
        });
    }
    ...
    /**
     * ── BLOG INTEGRATION ──
     * Shows the public-facing blog with articles + comments.
     * Lazily initializes blog services on first access.
     */
    @FXML
    public void showBlog() {
        System.out.println("DEBUG: AccueilFrontController.showBlog() called - FRONTEND BLOG");
        try {
            initBlogServices();

            if (frontBlogController == null) {
                frontBlogController = new FrontBlogController(blogArticleService, blogCommentService);
                blogView = frontBlogController.buildView();
            }
            frontBlogController.refresh();

            Button btnBack = new Button("← Retour au catalogue");
            btnBack.setStyle("-fx-background-color: transparent; -fx-text-fill: #5856d6;" +
                    " -fx-font-size: 13px; -fx-cursor: hand; -fx-padding: 10 16;");
            btnBack.setOnAction(e -> retourCatalogue());

            VBox wrapper = new VBox(btnBack, blogView);
            VBox.setVgrow(blogView, Priority.ALWAYS);
            ...
            alert.setTitle("Blog indisponible");
            alert.setContentText("Le blog n'a pas pu être chargé : " + e.getMessage());
            alert.showAndWait();
        }
    }
    ...
    private void loadStylesheet() {
        try {
            String cssPath = getClass().getResource("/gestion/AccueilFront.css").toExternalForm();
            if (contentStack.getScene() != null) {
                contentStack.getScene().getStylesheets().add(cssPath);
            }
        } catch (Exception e) {
    ...
    private void showUserProfile() {
        try {
            ...
            Views.UserProfileView profileView = new Views.UserProfileView(
                userController, faceAuthService, currentUser,
                this::retourCatalogue, // onShowUsers (simulate back)
                this::showUserProfile, // onUserUpdated
                this::retourCatalogue, // onShowLogs (not used here)
                this::logout,          // onLogout triggers global sign out
                this::retourCatalogue  // onShowHome
            );

            Region view = profileView.buildView();
            
            Button btnBack = new Button("← Retour à l'accueil");
            btnBack.setStyle("-fx-background-color: transparent; -fx-text-fill: #5856d6;" +
                    " -fx-font-size: 13px; -fx-cursor: hand; -fx-padding: 10 16;");
            btnBack.setOnAction(e -> retourCatalogue());
            ...
            contentStack.getChildren().setAll(wrapper);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    @FXML private void onRechercherProduit() { filtrerProduits(); }
    @FXML private void onReinitialiser()     { reinitialiserFiltres(); }
    ...
    private void validationChampPrix(TextField champ) {
        try {
            ...
            champ.setStyle(v < 0 ? "-fx-border-color: #dc3545; -fx-border-width: 2;"
    ...
    private boolean isRecent(java.util.Date date) {
        ...
        return (new java.util.Date().getTime() - date.getTime()) / (1000L * 60 * 60 * 24) <= 30;
    }
    ...
    private VBox creerProduitCard(Produit p) {
        ...
        try {
            if (p.getImage() != null && !p.getImage().isEmpty()) {
                ...
                iv.setImage(img.isError()
                        ? new Image("https://via.placeholder.com/180", 180, 180, true, true) : img);
            } else {
