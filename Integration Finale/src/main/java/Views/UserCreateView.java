...
public class UserCreateView {
    ...
    public UserCreateView(UserController userController, Runnable onBack, Runnable onSuccess) {
    ...
    public VBox buildView() {
        ...
        TextField fname = new TextField(); fname.getStyleClass().add("modern-input"); fname.setMinWidth(300);
        TextField lname = new TextField(); lname.getStyleClass().add("modern-input"); lname.setMinWidth(300);
        TextField email = new TextField(); email.getStyleClass().add("modern-input"); email.setMinWidth(300);
        PasswordField pwd = new PasswordField(); pwd.getStyleClass().add("modern-input"); pwd.setMinWidth(300);
        ...
        role.setValue(UserController.ROLE_NORMAL_USER);
        ...
        grid.add(createLabeledField("Password", pwd), 1, 1);
        grid.add(createLabeledField("User Role", role), 0, 2);

        statusLabel = new Label();
        statusLabel.setWrapText(true);

        Button saveBtn = new Button("Create User Account");
        saveBtn.getStyleClass().add("btn-primary");
        saveBtn.setPadding(new Insets(12, 30, 12, 30));
        saveBtn.setOnAction(e -> handleCreate(email.getText(), pwd.getText(), fname.getText(), lname.getText(), role.getValue()));
    ...
    private void handleCreate(String email, String pwd, String fname, String lname, String roleLabel) {
        if (email.isEmpty() || pwd.isEmpty() || fname.isEmpty()) {
            setError("Please fill in all required fields.");
            return;
        }

        User u = new User(0, email, "", pwd, fname, lname);
        try {
            userController.createUser(u, roleLabel);
            onSuccess.run();
        } catch (Exception ex) {
