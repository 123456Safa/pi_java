...
public class DetailReclamationControlleradmin {
    ...
    public void refreshReponses() {
        if (r != null) loadReponses(r.getId());
    ...
    public void refreshStatus(String newStatus) {
        if (r != null) {
            ...
            statut.setStyle("-fx-font-size: 12; -fx-font-weight: bold; -fx-text-fill: white; -fx-background-color: "
                    + getStatusColor(newStatus) + "; -fx-padding: 5 10; -fx-border-radius: 3;");
        }
    }
    ...
    private void deleteReponse(Reponse rep) {
        if (showDeleteConfirmation("Supprimer la réponse", "Êtes-vous sûr de vouloir supprimer cette réponse ?")) {
    ...
    public void repondre() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/reclamation/reponse-admin.fxml"));
            Parent root = loader.load();
    ...
    private void performTranslation(String targetLang, String langName) {
        if (r == null) return;
        if (translatedLangLabel != null) translatedLangLabel.setText(langName);
        if (translatedTitre != null) translatedTitre.setText("Traduction en cours...");
        if (translatedDescription != null) translatedDescription.setText("Traduction en cours...");
        if (translationContainer != null) { translationContainer.setVisible(true); translationContainer.setManaged(true); }
        new Thread(() -> {
            ...
            javafx.application.Platform.runLater(() -> {
                if (translatedTitre != null) translatedTitre.setText(titreTraduit);
                if (translatedDescription != null) translatedDescription.setText(descTraduit);
            });
        }).start();
    }
