...
public class FrontOfficeMainController {
    ...
    public void initialize() {
        ...
        // Auto-refresh notifications badge every 3 seconds
        javafx.animation.Timeline timeline = new javafx.animation.Timeline(
            new javafx.animation.KeyFrame(javafx.util.Duration.seconds(3), e -> updateNotificationsBadge())
        );
    ...
    private void updateNotificationsBadge() {
        if (notificationsBadge != null) {
            ...
            int count = ns.getUnreadCount(1); // Assuming user 1
    ...
    public void showReclamations() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/reclamation/home.fxml"));
            Parent view = loader.load();
            mainPane.setCenter(view);
            updateActiveButton(reclamationsButton);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void showNotifications() {
        loadVue("notifications", notificationsButton);
        // Hide badge when opened
        if (notificationsBadge != null) {
            notificationsBadge.setVisible(false);
            notificationsBadge.setText("0");
        }
    }

    private void loadVueHistorique() {
    ...
    private void loadVue(String fxml, Button activeButton) {
        try {
            ...
            if (controller != null && "HistoriqueController".equals(controller.getClass().getSimpleName())) {
                try {
    ...
    private void updateActiveButton(Button activeButton) {
        Button[] buttons = {catalogueButton, panierButton, historiqueButton, notificationsButton, reclamationsButton};
